...
class BasicCTCP
  ...
  ctcp :clientinfo
  def ctcp_version(m)
    m.ctcp_reply "SavageBot v#{VERSION} (Cinch v#{Cinch::VERSION})" if reply_to_ctcp?(:version)
  end
  ...
  def ctcp_source(m)
    m.ctcp_reply "http://github.com/SpenserJ/SavageBot" if reply_to_ctcp?(:source)
  end
